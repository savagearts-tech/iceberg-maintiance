...
class IcebergMaintenanceE2ETest {
    ...
    @Test
    @Order(6)
    @DisplayName("Verify --metadata-only cleanup mode")
    void testMetadataOnlyMode() {
        // Create an orphan metadata file
        String metadataOrphanKey = "e2e-test/e2e_db/e2e_tbl/metadata/orphan_m99.metadata.json";
        createOrphanFile(metadataOrphanKey);

        ScanOrphansCommand cmd = new ScanOrphansCommand(
                table, ((org.apache.iceberg.BaseTable) table).operations(), DATA_PREFIX, s3, null, null, true
        );
        cmd.execute();
        
        OrphanScanPipeline.Result result = cmd.getLastResult();
        
        System.out.println("Metadata-only prefixes: " + result.prefixes());
        assertEquals(1, result.prefixes().size(), "Should only have the metadata/ prefix");
        assertTrue(result.prefixes().get(0).endsWith("/metadata/"), "Prefix should be the metadata directory");
        
        assertTrue(result.dataOrphans().isEmpty(), "Data orphans should be empty in metadata-only mode");
        assertNull(result.referencedFiles(), "Referenced data files should be null");
        
        boolean foundOrphan = result.metadataOrphans().stream()
                .anyMatch(p -> p.contains("orphan_m99"));
        assertTrue(foundOrphan, "Should detect metadata orphan in metadata-only mode");
    }

    @Test
    @Order(7)
    @DisplayName("Verify TimeWindowPrefixGenerator with month partition transform")
    void testTimeWindowPrefixGeneratorMonth() {
        String monthTableName = "e2e_db.month_tbl";
        ...
        Table monthTable = catalog.createTable(
                TableIdentifier.parse(monthTableName), schema, spec,
                Map.of("format-version", "2"));
                
        String dataPrefix = WAREHOUSE + "/e2e_db/month_tbl/data";
        io.github.iceberg.cleanup.scan.TimeWindowPrefixGenerator generator = 
                new io.github.iceberg.cleanup.scan.TimeWindowPrefixGenerator(monthTable, dataPrefix);
                
        // Generate with 0 cooling days, 720 hours (30 days) scan window
        // This will definitely step across at least 1 or 2 month boundaries
        List<String> prefixes = generator.generate(0, 720);
        System.out.println("Generated month prefixes: " + prefixes);
        
        assertFalse(prefixes.isEmpty(), "Month prefixes should not be empty");
        assertTrue(prefixes.size() <= 2, "Should only generate 1 or 2 unique month prefixes for a 30-day window");
        
        java.util.regex.Pattern p = java.util.regex.Pattern.compile(".*event_date_month=\\d{4}-\\d{2}/");
        for (String prefix : prefixes) {
            assertTrue(p.matcher(prefix).matches(), "Prefix should match yyyy-MM format: " + prefix);
        }
        
        // cleanup
        catalog.dropTable(TableIdentifier.parse(monthTableName));
    }
}
